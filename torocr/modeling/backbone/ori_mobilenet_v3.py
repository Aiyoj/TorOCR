...
class ResidualUnit(nn.Module):
    def __init__(self, num_in_filter, num_mid_filter, num_out_filter, stride, kernel_size, act=None, use_se=False):
        ...
        self.conv1 = ConvBNACT(
            in_channels=num_mid_filter, out_channels=num_mid_filter, kernel_size=kernel_size,
            stride=stride, padding=int((kernel_size - 1) // 2), act=act, groups=num_mid_filter
        )
        if use_se:
...
class MobileNetV3(nn.Module):
    def __init__(self, config=None):
        ...
        self.disable_se = self.cfg.get("disable_se", True)
        self.model_name = self.cfg.get("model_name", "large")
        ...
        if self.model_name == "large":
            ...
            raise NotImplementedError("mode[" + self.model_name + "_model] is not implemented!")
        ...
        for layer_cfg in cfg:
            ...
            block = ResidualUnit(
                ...
                use_se=False if self.disable_se else layer_cfg[3]
            )
