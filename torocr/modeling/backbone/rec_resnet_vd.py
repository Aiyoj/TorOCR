...
class ResNetVd(nn.Module):
    def __init__(self, config=None):
        ...
        self.layers = self.cfg.get("layers", 34)
        ...
        assert self.layers in supported_layers, "supported layers are {} but input layer is {}".format(supported_layers,
                                                                                                       self.layers)
        ...
        self.conv1 = nn.Sequential(
            ConvBNACT(in_channels=self.in_channels, out_channels=32, kernel_size=3, stride=1, padding=1, act="relu"),
