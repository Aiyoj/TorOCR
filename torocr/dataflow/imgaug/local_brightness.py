...
class AugLocalBrightness(object):
    ...
    def __call__(self, data: dict):
        ...
        if random.random() > 0.5:
            image = self.add_parallel_light(image)
        else:
            image = self.add_spot_light(image)
    ...
    def generate_parallel_light_mask(self, mask_size,
        ...
        for i in range(canvas_y):
            if mode == "linear":
                ...
                i_value = self._decayed_value_in_norm(i, max_brightness, min_brightness, init_light_pos[1],
                                                      mask_size[1])
            else:
                i_value = 0
            mask[i] = i_value
